...
export default function ClientForm({ onClose }: ClientFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Para ambos tipos de pago, ahora se requiere número de cuotas
    if (!formData.numeroCuotas) {
      toast.error('Debe especificar el número de cuotas');
      return;
    }

    // Para cuotas, se requiere inicial
    ...
    const success = addClient(clientData);
    if (success) {
  ...
  return (
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Nuevo Registro de Cliente</DialogTitle>
        </DialogHeader>
